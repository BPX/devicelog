...
function Section({ children, className = '', alt = false, id }: { children: React.ReactNode, className?: string, alt?: boolean, id?: string }) {
  return (
    <section id={id} className={`py-24 sm:py-32 ${alt ? 'bg-slate-50 dark:bg-slate-900' : 'bg-white dark:bg-slate-950'} ${className}`}>
...
function Label({ children }: { children: React.ReactNode }) {
  return <p className="text-xs font-semibold tracking-[0.2em] uppercase text-cyan-600 dark:text-cyan-400 mb-4">{children}</p>
}
...
function PrimaryCTA({ href, children }: { href: string, children: React.ReactNode }) {
  return (
    <Link href={href} className="inline-flex items-center gap-2 px-6 py-3 bg-cyan-600 dark:bg-cyan-500 text-white text-sm font-semibold rounded-lg hover:bg-cyan-700 dark:hover:bg-cyan-400 transition-colors shadow-[0_0_0_1px_rgba(8,145,178,0.2),0_2px_4px_rgba(8,145,178,0.15)]">
...
function SecondaryCTA({ href, children }: { href: string, children: React.ReactNode }) {
  return (
    <Link href={href} className="inline-flex items-center gap-2 px-6 py-3 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 text-sm font-semibold rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border border-slate-200 dark:border-slate-700 shadow-[0_0_0_1px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.04)]">
      {children}
    </Link>
  )
}
...
const featureTabs = [
  {
    id: 'tracking', label: 'Asset Tracking', icon: Package,
    ...
    image: (
      <div className="bg-slate-900 rounded-lg p-6 border border-slate-700">
        ...
        <div className="flex -space-x-2">
          ...
          <div className="w-7 h-7 rounded-full border-2 border-slate-800 bg-slate-700 flex items-center justify-center text-[10px] font-semibold text-slate-400">+7</div>
        </div>
      </div>
    ),
  },
    ...
    image: (
      <div className="bg-slate-900 rounded-lg p-6 border border-slate-700 text-center">
        ...
        <div className="flex items-center justify-center gap-3 text-xs text-slate-400">
          <span>CSV</span><span className="text-slate-600">·</span>
          <span>XLSX</span><span className="text-slate-600">·</span>
          <span>JSON</span><span className="text-slate-600">·</span>
...
function CheckCell({ value }: { value: boolean | string }) {
  return (
    <span className="inline-flex items-center justify-center w-full">
      {value === true ? (<Check size={16} className="text-cyan-500 dark:text-cyan-400" />)
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 transition-colors">
      ...
      <nav className="border-b border-slate-100 dark:border-slate-800 sticky top-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md z-50">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
          ...
          <div className="hidden sm:flex items-center gap-5">
            ...
            <Link href="/signup" className="inline-flex items-center gap-1.5 px-4 py-2 bg-cyan-600 dark:bg-cyan-500 text-white text-sm font-semibold rounded-lg hover:bg-cyan-700 dark:hover:bg-cyan-400 transition-colors">
              Start free <ChevronRight size={14} />
            </Link>
          </div>
          <div className="flex items-center gap-2 sm:hidden">
            <button onClick={toggle} className="p-2 text-slate-500 dark:text-slate-400" aria-label="Toggle theme">
              {theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}
            </button>
            <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 text-slate-600 dark:text-slate-400" aria-label="Menu">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12h18M3 6h18M3 18h18" /></svg>
            </button>
          </div>
        </div>
        {mobileOpen && (
          <div className="sm:hidden px-6 pb-4 space-y-2 bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800">
            <Link href="/demo" className="block py-2.5 text-sm font-medium text-slate-600 dark:text-slate-400">Live Demo</Link>
            <Link href="/#pricing" className="block py-2.5 text-sm font-medium text-slate-600 dark:text-slate-400">Pricing</Link>
            <Link href="/login" className="block py-2.5 text-sm font-medium text-slate-600 dark:text-slate-400">Log in</Link>
            <Link href="/signup" className="block py-2.5 text-sm font-semibold text-cyan-600 dark:text-cyan-400">Start free →</Link>
          </div>
        )}
      </nav>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-24 sm:pt-32 pb-20 sm:pb-28">
        ...
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link href="/signup" className="inline-flex items-center gap-2 px-7 py-3.5 bg-cyan-600 dark:bg-cyan-500 text-white text-base font-semibold rounded-lg hover:bg-cyan-700 dark:hover:bg-cyan-400 transition-colors shadow-[0_0_0_1px_rgba(8,145,178,0.3),0_4px_8px_rgba(8,145,178,0.2)]">
            Start tracking for free <ArrowRight size={18} />
          </Link>
          <SecondaryCTA href="/demo">Try the live demo →</SecondaryCTA>
        </div>
        ...
        <div className="mt-16 max-w-5xl mx-auto">
            ...
            <div className="p-6 sm:p-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={s.label} className={`${s.bg} rounded-lg p-4 border border-white/5 text-left`}>
                  <div className="flex items-center gap-2 mb-3"><s.icon size={14} className={s.color} /><span className="text-[11px] text-slate-400 font-medium">{s.label}</span></div>
          ...
          <div className="text-center mt-6">
            <Link href="/demo" className="inline-flex items-center gap-1.5 text-sm font-semibold text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 dark:hover:text-cyan-300 transition-colors">
      ...
      {/* ── TRUST BAR ── */}
      <section className="border-y border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 py-12">
        <div className="max-w-6xl mx-auto px-6">
          <p className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-[0.15em] text-center mb-8 font-semibold">Trusted by IT teams worldwide</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            {[
              ...
              <div key={b.label} className="flex flex-col items-center text-center gap-2 p-4 rounded-lg hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-colors">
                <div className="w-10 h-10 bg-cyan-50 dark:bg-cyan-950 rounded-lg flex items-center justify-center mb-1">
                  <b.icon size={18} className="text-cyan-600 dark:text-cyan-400" />
                </div>
                <span className="text-sm font-semibold text-slate-800 dark:text-slate-200">{b.label}</span>
                <span className="text-xs text-slate-400 dark:text-slate-500">{b.sub}</span>
              </div>
            ))}
          </div>
          <div className="mt-10 flex flex-wrap items-center justify-center gap-x-10 gap-y-3">
            {['Startups','Agencies','Schools','Healthcare','Nonprofits','Remote Teams'].map(s => (
              <span key={s} className="text-sm font-semibold text-slate-500 dark:text-slate-400">{s}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <Section>
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 sm:p-8 border border-cyan-200 dark:border-cyan-800 shadow-[0_0_0_1px_rgba(8,145,178,0.15),0_4px_16px_rgba(8,145,178,0.08)]">
            <Label>THE SOLUTION</Label>
            <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-3">One dashboard. Your entire IT inventory. Always accurate.</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed mb-6">Every device logged with serial, warranty, and assigned owner. Certificate renewals tracked with email reminders. Your whole team sees the same data in real time.</p>
      ...
      {/* ── FEATURES ── */}
      <section className="py-24 sm:py-32 bg-slate-50 dark:bg-slate-900 transition-colors">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            <Label>FEATURES</Label>
            <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight text-slate-900 dark:text-slate-100 leading-tight">Everything you need, nothing you don't</h2>
            <Subtitle>Purpose-built for IT teams who want answers, not another tool to maintain.</Subtitle>
          </div>
          <div className="flex flex-wrap justify-center gap-1 mb-10 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
            {featureTabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveFeature(tab.id)}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
                  activeFeature === tab.id
                    ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-[0_0_0_1px_rgba(0,0,0,0.06),0_1px_2px_rgba(0,0,0,0.04)]'
          ...
          {featureTabs.filter(t => t.id === activeFeature).map(tab => (
            <div key={tab.id} className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                <h3 className="text-2xl font-semibold text-slate-900 dark:text-slate-100 tracking-tight mb-4">{tab.title}</h3>
                <p className="text-slate-500 dark:text-slate-400 leading-relaxed">{tab.description}</p>
              </div>
              <div>{tab.image}</div>
            </div>
          ))}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-[0_0_0_1px_rgba(0,0,0,0.04),0_4px_16px_rgba(0,0,0,0.04)]">
          <div className="overflow-x-auto">
            <table className="w-full table-fixed text-sm">
              <colgroup>
                <col className="w-[28%]" /><col className="w-[18%]" /><col className="w-[18%]" /><col className="w-[18%]" /><col className="w-[18%]" />
              </colgroup>
              <thead>
        ...
        <div className="grid sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
          {plans.map(plan => (
            <div key={plan.name} className={`rounded-xl p-8 flex flex-col border transition-colors ${
              plan.featured
                ? 'border-cyan-500 dark:border-cyan-400 bg-white dark:bg-slate-900 shadow-[0_0_0_1px_rgba(8,145,178,0.3),0_4px_16px_rgba(8,145,178,0.12)] relative'
                : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-[0_0_0_1px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.03)]'
            }`}>
              {plan.featured && <span className="absolute -top-3 left-1/2 -translate-x-1/2 inline-flex px-3 py-1 bg-cyan-600 dark:bg-cyan-500 text-white text-xs font-semibold rounded-full">Most popular</span>}
              ...
              <div className="mt-6 mb-8">
                <span className="text-4xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{plan.price}</span>
                {plan.period && <span className="text-slate-400 dark:text-slate-500 text-sm ml-1">{plan.period}</span>}
              </div>
              <ul className="space-y-3 mb-8 flex-1">
                {plan.features.map(f => (
                  <li key={f} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-400"><Check size={14} className="text-emerald-500 mt-0.5 flex-shrink-0" /> {f}</li>
                ))}
              </ul>
              <Link href={plan.href} className={`block text-center py-3 rounded-lg text-sm font-semibold transition-colors ${
                plan.featured ? 'bg-cyan-600 dark:bg-cyan-500 text-white hover:bg-cyan-700 dark:hover:bg-cyan-400' : 'border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'
              }`}>{plan.cta}</Link>
            </div>
          ))}
        </div>
      ...
      <Section>
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12"><Label>FAQ</Label><Heading>Got questions? We've got answers.</Heading></div>
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <details key={i} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 group shadow-[0_0_0_1px_rgba(0,0,0,0.04)] transition-colors">
                <summary className="px-6 py-4 cursor-pointer font-medium text-slate-900 dark:text-slate-200 list-none flex justify-between items-center gap-4">
                  <span>{faq.q}</span>
                  <ChevronRight size={16} className="text-slate-400 dark:text-slate-500 flex-shrink-0 group-open:rotate-90 transition-transform" />
                </summary>
      ...
      <section className="py-24 sm:py-32 bg-slate-900 dark:bg-slate-900">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link href="/demo" className="inline-flex items-center gap-2 px-7 py-3.5 border border-slate-600 text-slate-200 text-base font-semibold rounded-lg hover:bg-slate-800 transition-colors">
              Try the live demo →
            </Link>
          </div>
      ...
      {/* ── FOOTER ── */}
      <footer className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 py-16 transition-colors">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid sm:grid-cols-4 gap-8 mb-12">
            <div className="sm:col-span-1">
              <Link href="/" className="flex items-center gap-2 text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100"><img src="/logo.svg" alt="" className="w-7 h-7" />device<span className="text-cyan-600 dark:text-cyan-400">log</span></Link>
