...
export default function SettingsPage() {
  ...
  function addItem(field: 'categories'|'statuses'|'employees', value: string) {
    ...
    if (!settings[field].includes(value.trim())) {
      setSettings({ ...settings, [field]: [...settings[field], value.trim()].sort() })
    }
  }
  ...
  return (<div>
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5">
        ...
        <div className="flex gap-2">
          <input value={newCat} onChange={e => setNewCat(e.target.value)} placeholder="New category..."
            className="flex-1 px-3 py-1.5 border border-slate-300 rounded text-sm" onKeyDown={e => { if(e.key==='Enter'){addItem('categories',newCat);setNewCat('')} }} />
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5">
        ...
        <div className="flex gap-2">
          <input value={newStatus} onChange={e => setNewStatus(e.target.value)} placeholder="New status..."
            className="flex-1 px-3 py-1.5 border border-slate-300 rounded text-sm" onKeyDown={e => { if(e.key==='Enter'){addItem('statuses',newStatus);setNewStatus('')} }} />
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5">
        ...
        <div className="flex gap-2">
          <input value={newEmp} onChange={e => setNewEmp(e.target.value)} placeholder="Employee name..."
            className="flex-1 px-3 py-1.5 border border-slate-300 rounded text-sm" onKeyDown={e => { if(e.key==='Enter'){addItem('employees',newEmp);setNewEmp('')} }} />
      ...
      {showEmployeeImport && <CsvImport
        title="Import Employees"
        description="Upload a CSV with employee names. First column is used as the employee name."
        sampleData="John Smith\nJane Doe\nBob Wilson"
        sampleFilename="employees.csv"
        onImport={rows => { importEmployees(rows.map(r => Object.values(r)[0])); setSettings(getSettings()); setShowEmployeeImport(false) }}
